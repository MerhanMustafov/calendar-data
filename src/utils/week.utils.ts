import {
  IWeekDaysZeroBased,
  IWeekDaysNormal,
  IWeekDaysStringShort,
  IWeekDaysStringLong
} from '../interfaces/week.types';
import { weekDaysLookUpLong, weekDaysLookUpShort } from '../constants/week.constants';
 ...
 */
function weekDaysLookUpFactory(dayIndice: IWeekDaysZeroBased = 1): {
  [key in IWeekDaysZeroBased]: IWeekDaysNormal;
} {
  return weekDaysLookUpShort[dayIndice].reduce(
    (acc, { dayIndex }, i) => {
      return {
        ...acc,
        [dayIndex]: (i + 1) as IWeekDaysNormal
      };
    },
    {} as { [key in IWeekDaysZeroBased]: IWeekDaysNormal }
  );
}
...
export function getArrayOfWeekDaysStringShort(
  dayIndice: IWeekDaysZeroBased = 1
): IWeekDaysStringShort[] {
...
export function getFirstWeekEmptyDays(
  year: number,
  month: number,
  firstDayOfWeek: IWeekDaysZeroBased = 1
) {
  ...
  const day = _date.getDay() as IWeekDaysZeroBased;
